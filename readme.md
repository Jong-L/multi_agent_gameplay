...
Tiny Swords 是一款基于 Godot 引擎的 2D 俯视角四人混战竞技场游戏，旨在作为多智能体强化学习的博弈环境。四名角色（Agent）在一个封闭的竞技场中进行自由对抗，通过移动与近战攻击争夺分数。游戏在核心对抗之上叠加了少量环境交互元素（野生怪物、宝箱），以丰富策略空间并产生更具层次的博弈行为。

### 二、设计原则

策划遵循三条原则。第一，**博弈优先**：一切机制服务于产生清晰、可分析的多智能体博弈行为，避免引入难以量化或对训练收敛不利的随机性。第二，**最小可行**：优先实现能跑通"环境 → 训练 → 评估"闭环的最小功能集，额外效果以"插件式"方式逐步叠加，绝不阻塞主线开发。第三，**可观测性**：所有游戏状态均可被序列化为数值向量，便于作为强化学习的观测输入。

### 三、竞技场与比赛规则

#### 3.1 竞技场

竞技场为一个固定大小的矩形封闭地图（例如 20×15 逻辑格），四周由不可通行的墙壁围成。地图内部为平坦地形，不设复杂地形障碍，以降低路径规划的复杂度，让智能体的学习聚焦于对抗策略本身。四名角色分别在地图的四个角落出生。

#### 3.2 比赛流程

每局比赛持续固定时间步（例如 1000 步）。比赛结束时，按累计得分排名。角色被击杀后在随机位置复活，复活后有短暂无敌时间（约 15 步），期间不可被攻击也不可攻击他人，防止出生即死的恶性循环。

#### 3.3 胜负与得分

得分来源包括三个渠道：击杀其他玩家（+5 分）、击杀野生怪物（+2 分）、拾取宝箱（+1 分）。被击杀会扣分（-1 分），防止鲁莽冲撞。最终排名即为该局的博弈结果，用于计算强化学习的奖励信号。

### 四、角色与能力差异化设计

为了在博弈中产生**非对称均衡**，四名角色拥有不同的属性侧重。这种差异化是体现清晰博弈的关键——如果四个角色完全相同，策略空间会退化为对称博弈，行为趋同，缺乏观赏性和分析价值。通过差异化，智能体需要学习"扬长避短"以及"针对性对抗"，从而涌现出更丰富的策略组合。

四个角色的属性分配如下表所示：

| 角色 | 生命值 | 移动速度 | 攻击力 | 攻击范围 | 攻击冷却 | 设计意图 |
|------|--------|----------|--------|----------|----------|----------|
| **战士 (Warrior)** | 5 | 1.0 | 2 | 1.0 格 | 3 步 | 均衡型，无明显短板，适合正面交战 |
| **刺客 (Rogue)** | 3 | 1.5 | 3 | 0.8 格 | 4 步 | 高速高攻但脆皮，擅长hit-and-run |
| **骑士 (Knight)** | 7 | 0.7 | 1 | 1.2 格 | 2 步 | 高坦度长臂，适合阵地战与消耗 |
| **游侠 (Ranger)** | 4 | 1.2 | 2 | 1.5 格 | 5 步 | 最远攻击距离，适合风筝与牵制 |
...
**自身状态**（8 维）：自身 x 坐标、y 坐标、生命值比例、攻击冷却剩余比例、朝向（one-hot 编码 4 维）。

**其他三名角色状态**（每人 5 维，共 15 维）：相对 x 偏移、相对 y 偏移、生命值比例、是否存活、是否处于无敌状态。

**环境实体**（可变长度，固定最大数量后补零）：最近 N 个怪物的相对坐标与生命值（每个 3 维）、最近 M 个宝箱的相对坐标（每个 2 维）。

总观测维度约为 8 + 15 + 3N + 2M。以 N=3, M=3 为例，总维度为 38。

### 七、额外效果设计（轻量级）

以下额外效果经过筛选，满足"实现简单、不拖慢开发、能丰富博弈"的要求：

#### 7.1 野生怪物

竞技场中随机刷新少量野生怪物（同时存在 2-3 只）。怪物行为极其简单：在小范围内随机游走，被攻击时不还手，被击杀后掉落分数并在一段时间后重新刷新。怪物的作用是提供一个**公共资源争夺点**——智能体需要在"打怪刷分"和"对抗其他玩家"之间做出权衡，这本身就是一个经典的博弈决策（类似 MOBA 游戏中"刷野 vs 打架"的抉择）。

#### 7.2 宝箱

地图上随机位置刷新宝箱（同时存在 2-3 个），角色走到宝箱位置即自动拾取获得分数。宝箱的作用是鼓励智能体探索地图而非扎堆在一起，增加空间利用的策略维度。

#### 7.3 击杀奖励衰减

对同一目标的连续击杀会触发分数衰减（第一次 +5，第二次 +3，第三次 +1，之后不再得分，换目标后重置）。这一机制防止强势智能体反复欺负同一个弱势智能体，迫使其分散攻击目标，产生更复杂的多方博弈。实现上只需为每对 (attacker, victim) 维护一个计数器，非常轻量。

#### 7.4 不推荐加入的效果

以下效果虽然有趣但会显著增加开发复杂度，建议在基础版本稳定后再考虑：复杂地形与障碍物（需要寻路算法）、装备与道具系统（状态空间爆炸）、技能树与升级（训练周期大幅增长）、团队模式（需要额外的协作奖励设计）。
...
| 事件 | 奖励值 | 说明 |
|------|--------|------|
| 击杀其他玩家 | +5.0（含衰减） | 核心对抗激励 |
| 击杀野生怪物 | +2.0 | 资源争夺激励 |
| 拾取宝箱 | +1.0 | 探索激励 |
| 被击杀 | -1.0 | 生存惩罚 |
| 造成伤害 | +0.5 / 次 | 鼓励主动进攻 |
| 受到伤害 | -0.3 / 次 | 鼓励规避风险 |
| 存活（每步） | +0.01 | 微弱的生存激励，防止自杀策略 |

奖励值经过初步平衡，实际训练中可能需要根据智能体行为进行调参。
...
**第一阶段（核心闭环）**：实现竞技场地图、四个同质角色（暂不差异化）、移动与攻击、基础得分、与强化学习框架的通信接口。目标是跑通"Godot 环境 ↔ Python 训练脚本"的完整闭环。
...
│   │   └── Chest.tscn
│   ├── scripts/
│   │   ├── game_manager.gd         # 游戏逻辑主控
│   │   ├── character_base.gd       # 角色基类
│   │   ├── ai_controller.gd        # RL 桥接控制器
│   │   ├── monster.gd
│   │   └── chest.gd
│   └── assets/                     # 美术资源 (Tiny Swords 素材包)
│
├── python/                         # Python 训练侧
│   ├── envs/
│   │   ├── godot_env_wrapper.py    # Godot RL Agents 环境封装
│   │   └── pettingzoo_adapter.py   # PettingZoo Parallel API 适配器
│   ├── algorithms/
│   │   ├── ppo.py                  # PPO 算法实现
│   │   ├── networks.py             # 策略网络 & 价值网络定义
│   │   └── self_play.py            # Self-Play 对手池调度
│   ├── configs/
│   │   ├── default.yaml            # 默认超参数配置
│   │   ├── experiment/             # 实验配置组
│   │   └── sweep.yaml              # W&B Sweep 超参搜索配置
│   ├── evaluation/
│   │   ├── elo_rating.py           # Elo 评分计算
│   │   ├── metrics.py              # 博弈指标 (胜率矩阵/策略熵等)
│   │   └── replay_logger.py        # 对局回放记录
│   ├── dashboard/
│   │   ├── app.py                  # FastAPI 后端
│   │   └── frontend/               # Vue 前端 (对局回放/排名展示)
│   ├── train.py                    # 训练入口脚本
│   └── evaluate.py                 # 评估入口脚本
│
