...
每局比赛持续固定时间步，到时后所有智能体同时重置，完成一个回合。角色被击杀后在各自出生点复活

####  胜负与得分

得分来源包括三个渠道：攻击其他玩家、击杀野生怪物（敌人）、拾取奖励球。被攻击会扣分，随时间进行每秒加分或减分。
...
3. 造成伤害加分，稀疏对抗奖励
4. 受击减分，稀疏存活奖励
5. 塑形奖励：Policy Invariance Under Reward Transformations: Theory and Application to Reward Shaping提出了奖励塑形解决奖励稀疏加快训练的方法：

$$
R'(s, a, s') = R(s, a, s') + F(s, s')
$$

其中 $ F(s, s') = \gamma \Phi(s') - \Phi(s) $ 称为势能差，$ \Phi(s) $ 是状态 $ s $ 的势函数。

势能函数就是在原本的状态空间中加上了一个地形描述，指示哪里的状态是比较好的，事实上：

Potential-BasedShapingandQ-ValueInitializationare Equivalent指出使用势能函数参与奖励过程等价于对q值进行初始化,针对
$$
Q'(s,a)=Q_0(s,a)+\Phi(s)+\Delta Q'(s,a)\\
Q(s,a)=Q_0(s,a)+\Delta Q(s,a)
$$
其中$\Delta Q'(s,a),\Delta Q(s,a)$是在训练过程中Q值的变化量且有$\Delta Q'(s,a)=\Delta Q(s,a)$，$Q_0(s,a)$是初始值。

进一步有：
$$
V'(s)=\sum_a \pi(a|s)Q'(s,a)=\sum_a \pi(a|s)(Q(s,a)+\Phi(s))=V(s)+\Phi(s)
$$
使用势能函数进行奖励的环境实际上就是原本的状态值增加了势能函数的偏移值，这启示我们可以根据设计状态的势能函数，人为指出哪些状态是比较好的，让智能体倾向于往这些状态前进。
...
1. 发现边界撞墙行为明显，添加撞墙惩罚，撞到边界时如果还在移动，移动惩罚加倍
2. 
