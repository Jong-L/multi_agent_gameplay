...
每局比赛持续固定时间步。比赛结束时，按累计得分排名。角色被击杀后在随机位置复活，复活后有短暂无敌时间、

####  胜负与得分

得分来源包括三个渠道：击杀其他玩家（+10 分）、击杀野生怪物（+4 分）、拾取分数球（+1 分）。被击杀会扣分（-10 分），随时间进行每秒（-0.0分）。最终排名即为该局的博弈结果，用于计算强化学习的奖励信号。
...
**其他三名角色状态**（每人 5 维，共 15 维）：坐标、生命值、是否存活、是否处于无敌状态。
...
地图上随机位置刷新分数球，角色走到宝箱位置即自动拾取获得分数。

#### 击杀奖励衰减

对同一目标的连续击杀会触发分数衰减（10,5,2,0)。防止强势智能体反复欺负同一个弱势智能体。
