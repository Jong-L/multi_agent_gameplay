"""
Sparse Penalty (0.5) Excluded Analysis
Analyzes the Sparse Penalty (0.5) scheme with distance_penalty excluded from total score.
This helps understand the true performance without the distance penalty component.

Only processes LiDAR_no_shaping_0p5penalty files (5 files).
Note: wall_distance records are excluded from score calculation.

Generates:
1. Player average score (excluding distance penalty) vs episode
2. Total average score (excluding distance penalty) vs episode
3. Comparison with original scores
"""
...
def load_data_files(base_dir):
    """Load only LiDAR_no_shaping_0p5penalty files"""
    base_path = Path(base_dir)
    file_list = []
    
    for file in base_path.glob("*.csv"):
        fname = file.name
        if "LiDAR_no_shaping_0p5penalty" in fname:
...
def plot_total_comparison(stats, save_path=None, smooth_window=10):
    ...
    stats_text += f"With Distance Penalty: {df['total_original_score'].mean():.1f}\n"
    stats_text += f"Excluded: {df['total_excluded_score'].mean():.1f}\n"
    stats_text += f"Difference: {df['total_original_score'].mean() - df['total_excluded_score'].mean():.1f}"
...
def save_summary_csv(stats, output_dir):
    ...
    # Save episode stats
    output_path = summary_dir / "sparse_penalty_excluded_episode_stats.csv"
    ...
    summary_data = {
        ...
        'mean_total_original_score': df['total_original_score'].mean(),
        'mean_total_excluded_score': df['total_excluded_score'].mean(),
        'mean_total_wall_count': df['total_wall_count'].mean(),
        'score_difference': df['total_original_score'].mean() - df['total_excluded_score'].mean()
    }
    ...
    summary_path = summary_dir / "sparse_penalty_excluded_summary.csv"
...
def main():
    ...
    fig1 = plot_player_comparison(stats, 
                                  save_path=summary_dir / "sparse_penalty_excluded_player_comparison.png",
                                  smooth_window=10)
    
    print("  Creating total comparison...")
    fig2 = plot_total_comparison(stats, 
                                 save_path=summary_dir / "sparse_penalty_excluded_total_comparison.png",
                                 smooth_window=10)
    ...
    print("  - sparse_penalty_excluded_player_comparison.png")
    print("  - sparse_penalty_excluded_total_comparison.png")
    print("  - sparse_penalty_excluded_episode_stats.csv")
    print("  - sparse_penalty_excluded_summary.csv")
